...
class ConsultForm(BSForm):
    # consultant = forms.ChoiceField(disabled=True,)
...
class EnrollmentForm(BSForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.fields['consultant'].choices = [(self.instance.consultant.pk, self.instance.consultant), ]
        # 限制客户是当前销售的客户
        # self.fields['customer'].choices = [(customer.pk, str(customer)) for customer in
        #                                    self.instance.customer.consultant.customers.all()]
...
class CourseRecordForm(BSForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 限制老师是当前的用户
        self.fields['teacher'].choices = [(self.instance.teacher.pk, self.instance.teacher), ]
