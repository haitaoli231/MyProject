...
class EnrollmentList(View):
    ...
    def get(self, request):
        ...
        all_enrollment = models.Enrollment.objects.filter(q, delete_status=False, customer_id__in=[i.pk for i in
                                                                                                   request.user_obj.customers.all()])

        page = Pagination(request.GET.get('page'), all_enrollment.count(), request.GET.copy(), 10)
