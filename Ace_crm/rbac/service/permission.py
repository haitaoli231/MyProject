...
def init_permission(request, obj):  # 该函数接收request对象和当前登录的用户对象
    """初始化权限信息, 将权限信息放入session中保存起来, 该用户再次访问就可以直接访问自己权限内的信息了"""
    # 在数据库中查询出该用户对象的权限信息
    ...
    # 遍历查询出的权限信息permission_query,重构我们自定义的permission_dict和menu_dict
    for item in permission_query:
        # 将权限信息放入permission_dict
